...
@export var slope_uproll_adjuster: float
...
@export var dive_counterdirection_gravity_multiplier: float
...
func _physics_process(delta):
	...
	if is_on_floor():
		...
		if angler_dir != 0:
			...
			was_on_slope = false
		if skates_on == true:
			if angler_dir == -1 and velocity.x >= 0 and (angle * (180 / 3.141592)) >= 75 and slope_launched == false and not disable_slope_launch() and  can_uproll == false and downrolling == false:
				velocity.y = ((angle  * velocity.x ) * -1) / slope_uproll_adjuster
				can_wallrun_right = true
				#print("uprolling")
				possiblewallrun_timer.start()
				#$direction_change_timer.start()
			elif angler_dir == 1 and velocity.x <= 0 and (angle * (180 / 3.141592)) >= 75 and slope_launched == false and not disable_slope_launch() and can_uproll == false and downrolling == false:
				#print("uprolling")
				velocity.y = (angle  * velocity.x ) / slope_uproll_adjuster
		...
		if is_on_floor() or can_cayote_jump == true:
			...
			if was_on_slope == false:
				...
				if store_angler_dir == -1:
					if velocity.x >= 1:
						velocity.y = JUMP_VELOCITY* jump_multiplier - ((velocity.x/ counter_velocity_influence_adjust) * (fixed_angle/ counter_angle_influence_adjust))
						velocity.x = move_toward(velocity.x - (fixed_angle * 8) + (angle * angler_dir * 20), skating_SPEED * direction, accel)
					elif velocity.x <= 0:
						velocity.y = JUMP_VELOCITY + ((fixed_angle/ inward_angle_influence_adjust) * (velocity.x/ inward_velocity_influence_adjust) )
	...
	if skates_on == false and grabbing == false and grindin == false and wallkicking == false and knockedback == false:
		...
		if direction or do_dodgeslide == true:
			...
			if boost_mode != 0 and not is_on_floor():
				...
				store_running_speed = 0
				store_boost_mode = move_toward(store_boost_mode, 0, SPEED)
	...
	if skates_on == true and grindin == false and wallkicking == false and knockedback == false or grabbing == true:
		...
		if not is_on_floor():
			velocity.x = move_toward(velocity.x , skating_SPEED  * direction, accel)
		else:
			velocity.x = move_toward(velocity.x + (angle * angler_dir * 15), skating_SPEED  * direction, accel)
		#manage exeed mach
		skating_SPEED = skating_SPEED + (angle * (180 / 3.141592))/4 if abs(velocity.x) >= 900 and is_on_floor() else move_toward(skating_SPEED, 900, accel)
	...
	if not $wallkicklock1.is_colliding() and $wallkicklock1.enabled == true or not $wallkicklock2.is_colliding() and $wallkicklock2.enabled == true:
		store_wallkick_velocity = true
	if $wallkicklock1.is_colliding() and velocity.x != 0 and store_wallkick_velocity == true or $wallkicklock2.is_colliding() and velocity.x != 0 and store_wallkick_velocity == true:
	...
	if downrolling == true and is_on_floor() and can_walldive_left == false and can_walldive_right == false:
		#print("true")
	...
	if can_walldive_right == true:
		...
		if wallrunning_wallchecker.is_colliding():
			...
			if Input.is_action_pressed("right"):
				...
				wallrun_dive_gravity_multipier = dive_counterdirection_gravity_multiplier * wall_dive_gravity_multiplier
			else:
...
func set_launch_direction():
	slope_launch_direction = velocity.x * tan(deg_to_rad(smoothed_angle)) * angler_dir
