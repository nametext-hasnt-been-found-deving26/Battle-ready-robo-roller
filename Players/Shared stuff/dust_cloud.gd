...
@export var growth_multiplier: float = 1
...
func _process(delta: float) -> void:
	scale += Vector2(delta * growth_multiplier * scale.x, delta * growth_multiplier * scale.y)
	position += Vector2(scale.x * tan(rotation/2), scale.y /2 * tan(abs(rotation)))
