...
# Colors & dash blend
@export var color_inner_hollow := Color("#1d2a63")
@export var color_edge_hollow := Color("#000e5f")
@export var base_color_inner := Color("#0724a9")
@export var base_color_edge := Color("#051575")
var color_inner := base_color_inner
var color_edge  := base_color_edge
...
func _draw():
	...
	for i in range(points.size()):
		var global_p = points[i]
		var p = to_local(global_p)
		...
		left_side.append(p + perp * width)
		right_side.append(p - perp * width)

	if left_side.size() > 1:
		var ribbon: Array[Vector2] = []
		ribbon.append_array(left_side)

		right_side.reverse()
		ribbon.append_array(right_side)
		if ribbon.size() < 3:
			return

		var area := 0.0
		for i in range(ribbon.size()):
			var j = (i + 1) % ribbon.size()
			area += ribbon[i].x * ribbon[j].y
			area -= ribbon[j].x * ribbon[i].y

		if abs(area) < 0.01:
			return  # Polygon too small / collapsed — skip drawing
		if left_side.size() != right_side.size():
			return
		if left_side.size() < 2:
			return
		draw_polygon(ribbon, [color_inner])
		draw_polyline(ribbon + [ribbon[0]], color_edge, 1.0)
...
# show the computed anchor point (only for debugging)
	...
	if points.size() > 1:
		var diff = points[-2] - points[-1]
		if diff.length_squared() < 0.0001:
			return  # skip drawing tail this frame

		var tail_dir = diff.normalized()
...
func update_dash_color(dash_count: int) -> void:
	match dash_count:
		0:
			_target_inner = color_inner_hollow
			_target_edge  = color_edge_hollow
		1:
			_target_inner = base_color_inner
			_target_edge  = base_color_edge
		2:
...
func get_current_inner_color():
	return _target_inner
	
func get_current_edge_color():
	return _target_edge
