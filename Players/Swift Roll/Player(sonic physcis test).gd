...
func apply_main_movement(delta, direction):
	...
	if skates_on == true and grindin == false and wallkicking == false:
		...
		#if skates_on == true and abs(velocity.x) > Base_Skates_SPEED and velocity.x * tangent.angle() > 0 and angle > 0.01 and can_downroll == false and not jump_ball and ledge_titer_handler.full_colide:
...
func apply_vine_pull(delta):
	...
	var Spin_factor = grabbed_vine.Spin_factor
	var rotation_damp = grabbed_vine.rotation_damp
	...
	if Spin_factor/additive_damp >= 1.0  :
		tangential_vel *= Spin_factor/ additive_damp
		velocity = radial_vel + tangential_vel
		#print( tangential_vel.length(), "add ", additive_damp)
		additive_damp += rotation_damp
	#if not input_vector:
		#tangential_vel *= Spin_factor
		#velocity = radial_vel + tangential_vel
  ...
  # tweak this
	...
	#	if spring_vector.length() >= grabbed_vine.max_stretch:
		limit_brought_back =  (spring_vector.length() / grabbed_vine.max_stretch )#spring_vector.normalized() * 
			#print(abs(spring_vector.x) + abs(spring_vector.y))
		#print(limit_brought_back)
		#else:
			#limit_brought_back = 1
			#print(speed)
		if speed < min_speed:
			speed_dif = min_speed - speed
			var recovery_strength = 1.0 - (speed / min_speed)  # 0 → 1
			if velocity.y  >= 0 and handle_pos.y > global_position.y:
				velocity.y += gravity * delta * (recovery_strength) + (recovery_strength * speed_dif)
				#print("this is happening ", handle_pos.y - global_position.y)
