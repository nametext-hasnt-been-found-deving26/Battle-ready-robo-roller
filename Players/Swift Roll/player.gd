...
@export var store_dust_clouds = 0
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("jump") :
		if can_jump == true and can_walldive_left == false and can_walldive_right == false:
			jump_buffer = true
	...
	if is_on_floor():
		...
		if in_water == false and can_wallrun_left == false and can_wallrun_right == false:
			if skates_on == true and abs(velocity.x) > Base_Skates_SPEED and velocity.x * angler_dir > 0 and angle > 0.01 and can_downroll == false:
				...
				#print("works")
			elif abs(velocity.x) > Walking_SPEED and skates_on == false and angler_dir * velocity.x > 0:
